...
public class Simple_xy_rd {
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			assert shape[0] == NX;
			assert shape[1] == NY;
			System.out.println(dataVar.getDimension(0)+" "+dataVar.getDimension(1));
			String[] dim = dataVar.getDimensionsString().split(" ");
			...
			for (int j = 0; j < shape[0]; j++) {
				for (int i = 0; i < shape[1]; i++) {
					...
					System.out.printf("%-4d", dataIn[j][i]);
				}
				System.out.println();
			}
