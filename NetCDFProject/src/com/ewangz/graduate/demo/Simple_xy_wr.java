package com.ewangz.graduate.demo;
...
public class Simple_xy_wr {
	...
	public static void main(String args[]) {
		...
		String filename = "nc/simple_xy.nc";
		NetcdfFileWriter dataFile = null;		//netcdf文件写入类
		
		try {
			dataFile = NetcdfFileWriter.createNew(NetcdfFileWriter.Version.netcdf3, filename);
	
		    // Create netCDF dimensions,
		    Dimension xDim = dataFile.addDimension(null, "x", NX);		//netCDF=network Common Data Form,网络数据通用格式
		    Dimension yDim = dataFile.addDimension(null, "y", NY);
	
		    // define dimensions
		    List<Dimension> dims = new ArrayList<Dimension>();			//Dimension 维度
		    dims.add(xDim);
		    dims.add(yDim);
